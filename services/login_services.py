...
class LoginServices:
    ...
    async def login_user(email:str,password:str):
        ...
        if user.get("is_locked"):
            ...
            else:await users.update_one({"email":email},{"$set":{"is_locked":False,"failed_attempts":0,"lockout_time":None}}) # Unlock the user after cooldown
        ...
        await users.update_one({"email":email},{"$set":{"failed_attempts":0,"is_locked":False,"lockout_time":None}})
    ...
    async def change_password(username:str,old_password:str,new_password:str):
        ...
        if not user:return{"success":False,"message":"User not found."}
        if not await Security.verify_password(old_password,user["password"]):return{"success":False,"message":"Old password is incorrect."}
        hashed_new_password=await Security.hash_password(new_password)
        await users.update_one({"username":username},{"$set":{"password":hashed_new_password}})
