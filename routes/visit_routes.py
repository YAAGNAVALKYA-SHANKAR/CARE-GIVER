...
async def mark_arrival(visit_id:str,current_lat:float,current_long:float,current_user:dict=Depends(Security.get_current_user)):return await service.mark_arrival(visit_id,current_lat,current_long)
@router.get("/start-visit/{visit_id}")
